...
class RealDataLoader:
    ...
    def load_and_process(self) -> pd.DataFrame:
        ...
        df_filtered = df_raw[df_raw[consent_col].astype(str).str.upper().str.strip() == "ACEPTO PARTICIPAR"].copy()
        ...
        df_filtered = df_filtered[df_filtered[virtual_col].astype(str).str.strip().str.upper().isin(["SÍ", "SI"])].copy()
        ...
        df_proc['Universidad'] = df_filtered.iloc[:, 7].apply(self.clean_university)
        df_proc['Edad'] = np.nan
        df_proc['Sexo'] = np.nan
        df_proc['Semestre'] = np.nan

        # Ítems Likert de Alfabetización Mediática (C1-C10, T1-T10, P1-P10)
        # Se encuentran en las columnas del índice 9 al 38
        for i in range(1, 11):
            df_proc[f'C{i}'] = df_filtered.iloc[:, 9 + (i - 1)].apply(self.clean_likert_string)
        for i in range(1, 11):
            df_proc[f'T{i}'] = df_filtered.iloc[:, 19 + (i - 1)].apply(self.clean_likert_string)
        for i in range(1, 11):
            df_proc[f'P{i}'] = df_filtered.iloc[:, 29 + (i - 1)].apply(self.clean_likert_string)

        # Variables de Riesgo Académico (A1_Interrupcion, A2_Desaprobados, A3_Retirados, A4)
        df_proc['A1_Interrupcion'] = df_filtered.iloc[:, 51].apply(self.clean_a1)
        df_proc['A2_Desaprobados'] = df_filtered.iloc[:, 52].apply(self.clean_a2)
        df_proc['A3_Retirados'] = df_filtered.iloc[:, 53].apply(self.clean_a3)
        df_proc['A4'] = df_filtered.iloc[:, 54] # Bajo, Medio, Alto se mantiene directamente

        # Variables de Autoinforme y LMS Likert (A5-A8, L1-L8)
        # Se encuentran en las columnas del índice 55 al 66
        # CORRECCIÓN (Hallazgo 2.2): Se usan los nombres largos para alinear con scorer.py
        df_proc['A5_Dificultad'] = df_filtered.iloc[:, 55].apply(self.clean_likert_string)
        df_proc['A6_Consideracion_Abandono'] = df_filtered.iloc[:, 56].apply(self.clean_likert_string)
        df_proc['A7_Exigencia'] = df_filtered.iloc[:, 57].apply(self.clean_likert_string)
        df_proc['A8_Retrasos'] = df_filtered.iloc[:, 58].apply(self.clean_likert_string)

        df_proc['L1'] = df_filtered.iloc[:, 59].apply(self.clean_likert_string)
        df_proc['L2'] = df_filtered.iloc[:, 60].apply(self.clean_likert_string)
        df_proc['L3'] = df_filtered.iloc[:, 61].apply(self.clean_likert_string)
        df_proc['L4'] = df_filtered.iloc[:, 62].apply(self.clean_likert_string)
        df_proc['L5'] = df_filtered.iloc[:, 63].apply(self.clean_likert_string)
        df_proc['L6'] = df_filtered.iloc[:, 64].apply(self.clean_likert_string)
        df_proc['L7'] = df_filtered.iloc[:, 65].apply(self.clean_likert_string)
        df_proc['L8'] = df_filtered.iloc[:, 66].apply(self.clean_likert_string)

        # Marcadores ausentes en Excel real
        df_proc['Calidad_Percibida'] = np.nan

        # Mapear las 12 columnas cualitativas (del índice 39 al 50)
        ...
        for idx, col_name in enumerate(qual_col_names):
            df_proc[col_name] = df_filtered.iloc[:, 39 + idx].fillna("Sin respuesta.").astype(str)
